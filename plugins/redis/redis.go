...
func initRedis() {
	...
	logger.Infof("[initRedis] 初始化Redis...")

	c := config.C()
	cfg := &redis{}
	err := c.App("redis", cfg)
	if err != nil {
		logger.Error("[initRedis] %s", err)
	}

	if !cfg.Enabled {
		logger.Info("[initRedis] 未启用redis")
		return
	}

	// 加载哨兵模式
	if cfg.Sentinel != nil && cfg.Sentinel.Enabled {
		logger.Info("[initRedis] 初始化Redis，哨兵模式...")
		initSentinel(cfg)
	} else { // 普通模式
		logger.Info("[initRedis] 初始化Redis，普通模式...")
		initSingle(cfg)
	}

	logger.Info("[initRedis] 初始化Redis，检测连接...")
	...
	logger.Infof("[initRedis] 初始化Redis，检测连接Ping... %s", pong)
}
